...
export default function ShareBar({ title, slug }) {
  ...
  const shareUrl = typeof window !== "undefined"
    ...
    : `https://bjsracing.com/blog/${slug}`;
  const shareText = `${title} — BJS Racing`;
    ...
    <>
      ...
      <div className="md:hidden fixed bottom-24 right-4 z-50">
        {open && (
          <div
            ref={panelRef}
            className="absolute bottom-14 right-0 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 mb-2 w-48"
          >
            <a
              href={whatsappUrl}
              target="_blank"
            ...
            >
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${copied ? "bg-green-500" : "bg-orange-500"}`}>
                <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  {copied
                    ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
                    : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.367-5.656l-1.1 1.1" />
                  }
                </svg>
              </div>
        ...
        <button
          ref={btnRef}
          onClick={handleShare}
          className={`w-12 h-12 rounded-full shadow-lg text-white flex items-center justify-center active:scale-95 transition-all duration-200 cursor-pointer ${
            open ? "bg-slate-800 rotate-45" : "bg-orange-500"
          }`}
