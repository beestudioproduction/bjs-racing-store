...
export default function ShareBar({ title, slug }) {
  ...
  return (
    <>
      ...
      {/* ===== MOBILE: floating button + expandable panel (portal ke body) ===== */}
      {mounted && createPortal(
        ...
        >
          {/* Expandable panel */}
          {mobileOpen && (
            <div className="absolute bottom-14 right-0 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 mb-2 w-48">
              {/* WhatsApp */}
              <a
                href={`https://wa.me/?text=${encodeURIComponent(text + " " + buildUrl())}`}
                target="_blank"
